...
/**
 * Created by bobby.peng on 2016/11/9.
 */
public class BinaryTreeNodeUtils {
    ...
    private static String[] buildDataArray(String str) {
        str = str.replace("[", "");
        str = str.replace("]", "");
    ...
    public static TreeNode buildTreeByArray(Integer[] array) {
        ...
        buildTreeNode(treeNode, 0, array);

        return treeNode;
    }

    public static TreeNode buildTreeByString(String str) {

        return buildTreeByArray(transferStrToArray(str));
    }

    public static String transferTreeToStr(TreeNode treeNode) {
        // TODO: 2016/11/22
        return null;
    }

    public static List<Integer> transferTreeToArray(TreeNode treeNode) {
        List<Integer> result = new ArrayList<>();
        transferTreeToArray(treeNode, result, 0);
        return result;
    }

    public static void buildTreeInConsole(String str) {
        Integer[] array = transferStrToArray(str);
        ...
        int width = (int) Math.pow(2, layer - 1) * 8;

        for (int i = 1; i <= layer; i++) {
            int index = (int) Math.pow(2, i - 1) - 1;
            int nodeNum = (int) Math.pow(2, i - 1);

            int eachWidth = width / nodeNum;
            int blank = (eachWidth - 8) / 2;

            String format = "%" + blank + "s";

            String inLine = "";
            for (int j = index; j < index + nodeNum; j++) {
                if (j >= array.length) break;
//                if(array[j] == null) {
//                    inLine += String.format("%" + eachWidth + "s"," ");
//                    continue;
//                }

                if (blank == 0) {
                    inLine += String.format("%8s", array[j]);
                } else {
                    inLine += String.format(format, " ") + String.format("%8s", array[j]) + String.format(format, " ");
                }
            }
    ...
    private static void buildTreeNode(TreeNode node, int index, Integer[] array) {
        int leftNodeIndex = 2 * index + 1;
        if (node == null) {
            return;
        }
        if (leftNodeIndex < array.length && array[leftNodeIndex] != null) {
            TreeNode left = new TreeNode(array[leftNodeIndex]);
            node.left = left;
            buildTreeNode(left, leftNodeIndex, array);
        }

        if (leftNodeIndex + 1 < array.length && array[leftNodeIndex + 1] != null) {
            TreeNode right = new TreeNode(array[leftNodeIndex + 1]);
            node.right = right;
            buildTreeNode(right, leftNodeIndex + 1, array);
        }
    }

    private static Integer[] transferStrToArray(String str) {
        str = str.replace("[", "");
        str = str.replace("]", "");
